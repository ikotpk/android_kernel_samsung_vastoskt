...
static struct i2c_board_info fg_i2c_devices[] = {
  { 
    ...
    //.irq = MSM_GPIO_TO_INT( FUEL_ALERT ); 
  },
};
#endif
#if defined(CONFIG_I2C) && defined(CONFIG_GPIO_SX150X)
...
static int wlan_setup_ldo_33v(int input_flag, int on)
{
	...
	printk(KERN_INFO "%s - old wlan_33v_flag : %d\n",
			__func__, wlan_33v_flag);

	if (on) {
		if (wlan_33v_flag)  /* Already On */
			skip = 1;

		wlan_33v_flag |= input_flag;
	} else {
		wlan_33v_flag &= (~input_flag);
...
void wlan_setup_power(int on, int detect)
{
	...
	printk("%s %s --enter\n", __func__, on ? "on" : "down");
	printk(KERN_INFO "%s board_hw_revision : %d\n", __func__,
			board_hw_revision);
...
static struct snd_endpoint snd_endpoints_list[] = {
	...
	SND(MP3_SPEAKER_PHONE, 35),     // speaker except CALL
	SND(MP3_STEREO_HEADSET, 36),    // headset except CALL
	SND(BT_NSEC_OFF, 37),           // BT SCO NSEC OFF
	SND(HANDSET_VOIP, 38),
	SND(STEREO_HEADSET_VOIP, 39),
	SND(SPEAKER_VOIP, 40),
	SND(HANDSET_VOIP2, 41),
	SND(STEREO_HEADSET_VOIP2, 42),
	SND(SPEAKER_VOIP2, 43),
	SND(FM_ANALOG_STEREO_HEADSET, 46),
	SND(FM_ANALOG_STEREO_HEADSET_CODEC, 47),
	SND(CURRENT, 0x7FFFFFFE),
};
#undef SND
#endif
...
static void keypad_gpio_init(void)
{
	...
	printk("[KEY] %s start \n", __func__);
#if 0
	rc = gpio_request(36, "kbr(1)");
	if (rc < 0) {
		pr_err("[KEY] failed to request kbr(1)\n");
	}
	rc = gpio_request(37, "kbr(2)");
	if (rc < 0) {
		pr_err("[KEY] failed to request kbr(2)\n");
	}
	rc = gpio_request(39, "kbr(4)");
	if (rc < 0) {
		pr_err("[KEY] failed to request kbr(4)\n");
	}
	rc = gpio_request(31, "kbc(0)");
	if (rc < 0) {
		pr_err("[KEY] failed to request kbc(0)\n");
	}	
#endif

#if 1
	gpio_tlmm_config(GPIO_CFG(36, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
//	printk("[KEY] kbr(1) gpio_36:  input, pull up \n");
	gpio_tlmm_config(GPIO_CFG(37, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
//	printk("[KEY] kbr(2) gpio_37:  input, pull up \n");
	gpio_tlmm_config(GPIO_CFG(39, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
//	printk("[KEY] kbr(4) gpio_39:  input, pull up \n");
	gpio_tlmm_config(GPIO_CFG(31, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
//	printk("[KEY] kbc(0) gpio_31:  output, no pull\n");
#else
	gpio_tlmm_config(GPIO_CFG(36, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	printk("[KEY] kbr(1) gpio_36:  output, no pull\n");
	gpio_tlmm_config(GPIO_CFG(37, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	printk("[KEY] kbr(2) gpio_37:  output, no pull\n");
	gpio_tlmm_config(GPIO_CFG(39, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	printk("[KEY] kbr(4) gpio_39:  output, no pull\n");
	gpio_tlmm_config(GPIO_CFG(31, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	printk("[KEY] kbc(0) gpio_31:  input, pull up\n");
#endif
}
