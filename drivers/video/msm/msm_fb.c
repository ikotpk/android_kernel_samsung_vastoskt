...
#define INIT_IMAGE_FILE		"/VASTO.rle"
#define INIT_LPM_IMAGE_FILE   "/VASTO_LPM.rle"
#else
...
#define PROGRESS_BAR_DRAW_ONCE 5
#else
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	MSM_FB_INFO
	    ("FrameBuffer[%d] %dx%d size=%d bytes is registered successfully! (%d bits/pixel)\n",
	     mfd->index, fbi->var.xres, fbi->var.yres, fbi->fix.smem_len, fbi->var.bits_per_pixel);

#ifdef CONFIG_FB_MSM_LOGO
#ifdef CONFIG_BATTERY_SEC  
		if (is_lpcharging_state() == 1) {
			logofilename = INIT_LPM_IMAGE_FILE;
...
#ifdef DISPLAY_BOOT_PROGRESS
#if defined(CONFIG_MACH_VASTO)
	if ((in_recovery_mode!=1)
#ifdef CONFIG_BATTERY_SEC
	&& !is_lpcharging_state() 
#endif
	)
#endif
	msm_fb_start_progress(fbi);
#endif
