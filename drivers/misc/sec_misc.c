...
#ifdef SALES_CODE
static ssize_t sales_code_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int sales_code;
...
static ssize_t sales_code_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int sales_code;
...
static DEVICE_ATTR(sales_code, S_IRUGO | S_IWUGO, sales_code_show, sales_code_store);
#endif /*SALES_CODE*/
