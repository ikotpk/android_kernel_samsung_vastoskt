...
INC_UINT8 INC_CHIP_STATUS(INC_UINT8 ucI2CID)
{
    ...
    if((uiChipID & 0xF00) != 0xC00)
    {
        printk("[INC] INC_CHIP_STATUS--> INC_ERROR uiChipID %d\n", uiChipID);
        return INC_ERROR;
    }
    return INC_SUCCESS;
}
...
/*
    for(i=0x0; i<0x64; i++)
    {
        INC_CMD_WRITE(test_ucI2CID, APB_MPI_BASE+ 0x01, i);
        INC_CMD_READ(test_ucI2CID, APB_MPI_BASE+ 0x01);
        printk("[INC] EBI TEST %x", test_ucI2CID);
    }
*/
    ...
    {
        printk("[INC] INC_PLL_SET--> INC_ERROR\n");
        return INC_ERROR;
    }
    if(INC_CHIP_STATUS(ucI2CID) != INC_SUCCESS)
    {
        printk("[INC] INC_CHIP_STATUS--> INC_ERROR\n");
        return INC_ERROR;
    }
    ...
    INC_DELAY(100);
    printk("[INC] INC_INIT--> INC_SUCCESS\n");
    return INC_SUCCESS;
}
...
INC_UINT8 INC_READY(INC_UINT8 ucI2CID, INC_UINT32 ulFreq)
{
    ...
    if(INC_RF500_START(ucI2CID, ulFreq, m_ucRfBand) != INC_SUCCESS){
        ...
        printk("[T3700] INC_READY INC_ERROR\n");
        return INC_ERROR;
    }
...
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
    ...
    //printk("[T3700] INC_SYNCDETECTOR INC_ERROR pInfo->nBbpStatus %x , wData %x\n", pInfo->nBbpStatus, wData);
    return INC_ERROR;
}
...
INC_UINT8 INC_FICDECODER(INC_UINT8 ucI2CID, ST_SIMPLE_FIC bSimpleFIC)
{
    ...
    printk("[T3700] INC_FICDECODER INC_ERROR\n");
	return INC_ERROR;
}
...
INC_UINT8 INC_STOP(INC_UINT8 ucI2CID)
{
	...
	if(uStatus == 0x0011){
		...
		if(nLoop >= 10){
            ...
            printk("[T3700] INC_STOP INC_ERROR\n");
			return INC_ERROR;
		}
	}
	...
	if(nLoop >= 10){
		pInfo->nBbpStatus = ERROR_STOP;
        printk("[T3700] INC_STOP INC_RETRY\n");
		return INC_RETRY;
	}
...
INC_UINT8 INC_GET_VTBER(INC_UINT8 ucI2CID)
{
    INC_UINT16  uiRSErrBit;
    INC_UINT16  uiRSErrTS;
    INC_UINT16  uiError, uiRef;
...
#else//#if 1 //Neo.shyoon0502.I&C ¨öCA©ª¡Æ¨£ Au¢¯e 
        {105,912}, //930
        {100,860}, //910
        {95,816},  //880
        {90,770},  //845
...
INC_UINT16 INC_GET_SAMSUNG_BER(INC_UINT8 ucI2CID)
{
    ...
    printk("[T3700]INC_nBER %d, INC_vBER %d", nBER, vBER);
